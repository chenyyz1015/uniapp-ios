...
typedef NS_ENUM(NSInteger, TXVideoRenderMirrorType) {
    ///自动模式，前置摄像头是镜像，后置非镜像
    VIDEO_RENDER_MIRROR_TYPE_AUTO = 0,
    ///强制开启镜像，无论当前使用的是前置摄像头还是后置摄像头
    VIDEO_RENDER_MIRROR_TYPE_ENABLE = 1,
    ///强制关闭镜像，不论当前使用的是前置摄像头还是后置摄像头
    VIDEO_RENDER_MIRROR_TYPE_DISABLE = 2
};
...
typedef NS_ENUM(NSInteger, TXAudioChannel) {
  AUDIO_CHANNEL_MONO = 1,   // 单声道
  AUDIO_CHANNEL_STEREO = 2  // 双声道（立体声）
};
...
/// 语音录制参数类
LITEAV_EXPORT @interface TXUGCAudioConfig : NSObject
/// 音频采样率
@property(nonatomic, assign) TXAudioSampleRate audioSampleRate;
/// 声道数 （可选单声道或者双声道）
@property(nonatomic, assign) TXAudioChannel audioChannel;
/// 音频码率(bps)
@property(nonatomic, assign) int audioBitrateBps;
/// 设置录制的最小时长(ms), 大于0
@property(nonatomic, assign) float minDurationMs;
/// 设置录制的最大时长(ms)
@property(nonatomic, assign) float maxDurationMs;
/// 是否开启语音AI降噪，默认为false
@property(nonatomic, assign) BOOL enableAIDeNoise;
@end

/**
 * 开始语音录制结果错误码定义
 */
typedef NS_ENUM(NSInteger, TXUGCStartAudioRecordResultCode) {
  /// 开始录制成功
  START_AUDIO_RECORD_OK  = 0,
  /// 录制文件路径为空
  START_AUDIO_RECORD_ERROR_PATH_IS_EMPTY  = -2,
  /// 麦克风当前无法使用
  START_AUDIO_RECORD_ERROR_MIC_CANNOT_USE = -4,
  /// 鉴权失败
  START_AUDIO_RECORD_ERROR_LICENCE_VERIFICATION_FAILED = -5,
  /// 调用API时传入的参数不合法
  START_AUDIO_RECORD_ERROR_INVALID_PARAMETER = -6
};
...
typedef NS_ENUM(NSInteger, TXUGCRecordResultCode) {
  ...
  /// 录制成功 但是没有音频流
  UGC_RECORD_RESULT_OK_WITHOUT_AUDIO = 4,
  /// 录制失败，不会生成最终视频
